...
our $start_param = '01/01/2009';     # Start value for plot, untainted input parameter.
our $end_param = '12/31/2009';       # End value for plot, untainted input parameter.
...
sub untaint_params {
  ...
  my ($start)    = $q->param('start') =~ /^(\d\d?\/\d\d?\/\d\d\d\d)$/;
  my ($end)      = $q->param('end') =~   /^(\d\d?\/\d\d?\/\d\d\d\d)$/;
...
sub generate_chart {
  ...
  $graph->set( title             => $title,
               ...
               x_label           => 1,
               x_label_skip      => 5,
               x_tick_offset     => (scalar($#{$p_data->[0]}) % 5),
