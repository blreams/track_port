...
def get_options_info(symbol, expiration='', columnsToRetrieve ='*'):
    """Retrieves options data for the provided symbol or symbol list."""

    if isinstance(symbol, basestring): symbol = [symbol]
    columns = ','.join(columnsToRetrieve)
    symbolspec = '(' + ' or '.join(['symbol = \''+s+'\'' for s in symbol]) + ')'
    yql = 'select %s from %s where %s' \
          % (columns, FINANCE_TABLES['options'], symbolspec)

    if expiration != '':
        if isinstance(expiration, basestring): expiration = [expiration]
        expirationspec = '(' + ' or '.join(['expiration = \''+s+'\'' for s in expiration]) + ')'
        yql += " and %s" %(expirationspec)
...
def get_multi_options_info(symbolexp, columnsToRetrieve ='*'):
    """Retrieves options data for the provided tuple symbol,expiration or list of tuples."""

    if isinstance(symbolexp, tuple): symbolexp = [symbolexp]
    symbolexpspec = ' or '.join(['%s and %s' % ('symbol = \''+s+'\'', 'expiration = \''+e+'\'') for s, e in symbolexp])
    columns = ','.join(columnsToRetrieve)
    yql = 'select %s from %s where %s' \
          % (columns, FINANCE_TABLES['options'], symbolexpspec)
